...
import org.jetbrains.plugins.terminal.JBTerminalSystemSettingsProviderBase
import org.jetbrains.plugins.terminal.JBTerminalWidget
import org.jetbrains.plugins.terminal.TerminalExecutionConsole
import org.jetbrains.plugins.terminal.util.PtyProcessTtyConnector
...
class TerminalProcessService(private val project: Project) : com.intellij.openapi.Disposable {
    ...
    fun initialize(workingDir: String? = System.getProperty("user.home")): Boolean {
        ...
        this.workingDir = wd
        val cmd = if (SystemInfo.isWindows) arrayOf("cmd.exe") else arrayOf("/bin/bash", "-l")
        return try {
            pty = PtyProcess.exec(cmd, emptyMap(), wd)
            ...
            ApplicationManager.getApplication().invokeAndWait {
                try {
                    ...
                    val console = TerminalExecutionConsole(project, false)
